...
public class DelayedService extends Thread {
    ...
    public static Task addCalculateTask(Task task, boolean isGenerateId) {
        if (GmsUtil.StringNotNull(task.getTaskId())) {
            addTask(task, 0);
            return task;
        }
        if (isGenerateId) {
    ...
    private void updateKey(List<Task> tasks) {//更新下次任务执行时间,删除原有任务，添加新任务
        if (!tasks.isEmpty()) {
            for (Task task : tasks) {
                if (task.isNeedKeep()) {
                    RedisService.set(StaticConfig.KEEP_DB, RedisKeyPool.DELAY_TASK_PREFIX + task.getTaskId(), GmsUtil.toJson(task));
                }
    ...
    public static Task buildTask() {//创建好任务后,可以在添加任务前覆盖自动生成的taskId
        String id;
        do {
            id = generateId();
    ...
    public static class Task {
        ...
        private void updateNextTime() {
            ...
            do {
                ...
                time += delay;
            } while (!this.repetition && this.nextTime < GmsUtil.getCurTime());
        }
    ...
    @PostConstruct
    public void test() {
        DelayedService.Task task = DelayedService.Task.buildTask();//有返回值的样例
        task.withTask(() -> {
            int exec = exec();
            task.putResult(exec);
        }).withNum(10000);
        DelayedService.addInitializedTask(task,20);

        DelayedService.Task task1 = DelayedService.buildTask()
                .withPrintLog(true)
                .withDesc("测试123")
                .withDelay(1000)
                .withCalculate("com.zs.gms.service.init.GmsStartedUpRunner", "time", new Object[]{})
                .withKeep(true)
                .withNum(100)
                .withMethodTask("com.zs.gms.service.init.GmsStartedUpRunner","print",new Object[]{})
                .withAtOnce(true)
                .withTask(()->{
