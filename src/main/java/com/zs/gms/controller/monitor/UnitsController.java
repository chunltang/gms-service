...
public class UnitsController extends BaseController {
    ...
    public String getUnits() throws GmsException {
        try {
            ...
            List<DispatchTask> dispatchTasks = dispatchTaskService.getDispatchTaskList(currentUser.getUserId(), UnitTypeEnum.LOAD_DISPATCHTASK, mapId);
            return GmsUtil.toJsonIEnumDesc(new GmsResponse().data(dispatchTasks).message("获取调度员管理的调度单元成功").success());
        }catch (Exception e){
